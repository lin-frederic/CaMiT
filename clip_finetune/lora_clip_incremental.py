...
from models import get_model_with_lora, get_model_with_lora_from_model
...
class ModelWithHead(nn.Module):
    ...
    def update_head(self,num_classes):
        ...
        new_head = CosineLinear(self.head.in_features, num_classes)
        with torch.no_grad():
            new_head.weight.data[:old_weight.size(0), :] = old_weight
...
def train_epoch(model, train_loader, optimizer, scheduler, writer, epoch):
    ...
    for i, (images, labels) in tqdm(enumerate(train_loader), total=len(train_loader), desc="Training", unit="batch"):
        ...
        if i % 10 == 0:
            ...
            writer.add_scalar("Train/Accuracy", correct / max(total, 1) * 100, step) # avoid division by zero
...
if __name__=="__main__":
    ...
    parser.add_argument("--model_size", type=str, default="b", help="Model size: b or l")
    ...
    parser.add_argument("--model_2007_path", type=str, default="lora_clip/checkpoints_b/checkpoint_19.pth", help="Path to model checkpoint for 2007")
    

    parser.add_argument("--save_dir", type=str, default="lora_clip/checkpoints", help="Directory to save checkpoints")
    parser.add_argument("--log_dir", type=str, default="lora_clip/runs", help="Directory to save logs")

    args = parser.parse_args()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model_size = args.model_size
    model, preprocess_train, preprocess_val = get_model_with_lora(model_size)

    # Load the datasets
    train_dataset = SupervisedTimeDataset(args.train_annotations, args.train_images_dir, "2007", transform=preprocess_train)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)

    test_dataset = SupervisedTimeDataset(args.test_annotations, args.test_images_dir, "2007", transform=preprocess_val)
    ...
    class_order.update(train_dataset.class_to_idx.keys())
    ...
    # Load 2007 model
    state_dict = torch.load(args.model_2007_path, map_location="cpu")
    model.load_state_dict(state_dict)
    base_model = get_model_with_lora_from_model(model.base_model)
    model.base_model = base_model

    model = model.to(device)

    train_years = sorted(os.listdir(args.train_images_dir), key=lambda x: int(x)) # 2007-2023: 17 years
    train_years = train_years[1:] # 2008-2023

    for i, year in enumerate(train_years):
        print("Training on year:", year)
 
        train_dataset = SupervisedTimeDataset(args.train_annotations, args.train_images_dir, year, transform=preprocess_train, class_order=class_order) 
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
        test_dataset = SupervisedTimeDataset(args.test_annotations, args.test_images_dir, year, transform=preprocess_val, class_order=class_order)
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
        class_order.update(train_dataset.class_to_idx.keys())
        num_classes = len(class_order)
        print(f"Number of classes: {num_classes}")
        ...
        # check if the year is already trained
        if os.path.exists(os.path.join(args.save_dir, f"checkpoint_{year}_{args.num_epochs-1}.pth")):
            # load the model
            print(f"Model for year {year} already trained. Loading the model.")
            state_dict = torch.load(os.path.join(args.save_dir, f"checkpoint_{year}_{args.num_epochs-1}.pth"), map_location="cpu")
            model.load_state_dict(state_dict)
            continue
        ...
        logdir = os.path.join(args.log_dir, f"runs_{year}")
        ...
        for epoch in range(args.num_epochs):
            ...
            torch.save(model.state_dict(), os.path.join(args.save_dir, f"checkpoint_{year}_{epoch}.pth"))

        if i < len(train_years) - 1: # if not the last year, prepare for the next year
            base_model = get_model_with_lora_from_model(model.base_model) # merge_unload current lora, create new lora for next year
            model.base_model = base_model
