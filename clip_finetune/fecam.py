...
from dataset_for_incremental import SupervisedTimeDataset
from models import get_model, load_embeddings
from utils import parse_args, get_years
...
def compute_prototypes_and_second_moments(embeddings, labels):
    """
    ...
    prototypes = {} # E[X]
    second_moments = {} # E[X^2]
    for label in tqdm(unique_labels, desc="Computing prototypes and second moments"):
        indices = [i for i, l in enumerate(labels) if l == label]
        normalized_embeddings = F.normalize(embeddings[indices], dim=1)
        prototypes[label] = torch.mean(normalized_embeddings, dim=0) # (D,)
        second_moments[label] = torch.mean(normalized_embeddings.unsqueeze(2) * normalized_embeddings.unsqueeze(1), dim=0) # (D, D)

    return prototypes, second_moments


def update_statistics(old_prototype, old_second_moment, old_count, new_prototype, new_second_moment, new_count):
    """
    Update the prototype, second moment and count incrementally.
    """
    n_old = old_count
    n_new = new_count
    n_total = n_old + n_new

    updated_prototype = (n_old * old_prototype + n_new * new_prototype) / n_total
    updated_second_moment = (n_old * old_second_moment + n_new * new_second_moment) / n_total

    return updated_prototype, updated_second_moment, n_total

def compute_covariance(mean, second_moment):
...
if __name__=="__main__":
    args = parse_args()
    assert args.num_jobs == 1, "Only one job is supported for FeCAMv3"
    ...
    # mu_{new} = (n_old * mu_{old} + n_new * mu_{new}) / (n_old + n_new)
    # cov_{new} =  (n_old * E[X_old^2] + n_new * E[X_new^2] -(n_old+n_new) * mu_{new}^2) / (n_old + n_new)
    averaged_prototypes = {} # E[X]
    averaged_second_moments = {} # E[X^2]
    counts = {} # n
    ...
    with tqdm(total=len(train_years) * len(test_years)) as pbar:
        for train_year in train_years:
            ...
            prototypes, second_moments = compute_prototypes_and_second_moments(train_embeddings, train_labels)
            ...
            for label in prototypes.keys():
                if label not in averaged_prototypes:
                    ...
                    averaged_second_moments[label] = second_moments[label]
                    counts[label] = len([l for l in train_labels if l == label])
                else:
                    averaged_prototypes[label], averaged_second_moments[label], counts[label] = update_statistics(averaged_prototypes[label],
                                                                                                                  averaged_second_moments[label], 
                                                                                                                  counts[label], 
                                                                                                                  prototypes[label], 
                                                                                                                  second_moments[label], 
                                                                                                                  len([l for l in train_labels if l == label]))
            # Compute covariance matrices
            covariances = {}
            inv_covariances = {}
            for label in averaged_prototypes.keys():
                covariances[label] = compute_covariance(averaged_prototypes[label], averaged_second_moments[label])
    ...
    # Save results
    os.makedirs("fecamv3_results", exist_ok=True)
    os.makedirs(f"fecamv3_results/{model_name}", exist_ok=True)
    with open(os.path.join(f"fecamv3_results/{model_name}", f"results_{args.job_id}.json"), "w") as f:
