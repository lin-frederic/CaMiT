...
def train_epoch(model, train_loader, optimizer, scheduler, writer, epoch):
    ...
    for i, (images, labels) in tqdm(enumerate(train_loader), total=len(train_loader), desc="Training", unit="batch"):
        ...
        if i % 10 == 0:
            ...
            writer.add_scalar("Train/Accuracy", correct / max(total, 1), step)  # avoid division by zero
            print(f"Step {step}: Loss: {loss.item():.4f}, Accuracy: {correct / max(total, 1)*100:.2f}%")
...
if __name__=="__main__":
    ...
    parser.add_argument("--model_path", type=str, default="initial_pretrain_with_train/checkpoints/checkpoint_0199.pth.tar", help="Path to pre-trained model checkpoint")
    ...
    parser.add_argument("--model_2007_path", type=str, default="lora_mocov3/checkpoints_s/checkpoint_19.pth", help="Path to model checkpoint for 2007")
    ...
    for i, year in enumerate(train_years):
        ...
        if os.path.exists(os.path.join(args.save_dir, f"checkpoint_{year}_{args.num_epochs-1}.pth")):
            ...
            model.load_state_dict(torch.load(os.path.join(args.save_dir, f"checkpoint_{year}_{args.num_epochs-1}.pth")))
            continue
