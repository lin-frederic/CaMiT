...
if __name__=="__main__":
    ...
    os.makedirs(f"features/{args.pretraining_time}", exist_ok=True)
    os.makedirs(f"features/{args.pretraining_time}/{model_name}_current", exist_ok=True)
    with tqdm(total=len(train_years) * len(test_years)) as pbar:
        for train_year in train_years:
            print(f"Processing train year {train_year}")
            features_path = os.path.join(f"features/{args.pretraining_time}/{model_name}_current", f"features_train_{train_year}.pt")
            ...
            for test_year in test_years:
                features_path = os.path.join(f"features/{args.pretraining_time}/{model_name}_current", f"features_test_{test_year}.pt")
                if not os.path.exists(features_path):
    ...
    os.makedirs(f"ncm_results/{model_name}_{args.pretraining_time}_current", exist_ok=True)
    with open(f"ncm_results/{model_name}_{args.pretraining_time}_current/ncm_results_{args.job_id}.json", "w") as f:
