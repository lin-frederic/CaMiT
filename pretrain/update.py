...
import vits
from dataset import PretrainDatasetUpdate
...
parser.add_argument("--year", type=int, help="Year to pretrain on", default=2008)
...
parser.add_argument("--train_data", type=str, help="Path to train data", default=os.path.join(os.environ['HOME'], 'cars_dataset/train_crops/2007'))
...
parser.add_argument('--logdir', default='initial_pretrain', type=str)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.rank == 0:
        ...
        os.makedirs(os.path.join(args.logdir, "runs"), exist_ok=True)
        summary_writer = SummaryWriter(os.path.join(args.logdir, "runs"))
    else:
        summary_writer = None
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            # for update, don't use the epoch in the checkpoint
            ...
            try:
                ...
                print("=> no optimizer state_dict found in checkpoint")
                print("=> Using default optimizer")
            
            try:
                scaler.load_state_dict(checkpoint['scaler'])
                print("=> loaded scaler state_dict from checkpoint")
            except:
                print("=> no scaler state_dict found in checkpoint")
    ...
    train_dataset = PretrainDatasetUpdate(pretrain_dir=args.pretrain_data,
    ...
    args.epochs = 300 # hardcoded for now
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            filename = os.path.join(args.logdir, 'checkpoint_%04d.pth.tar' % epoch)
            save_checkpoint({
                'epoch': epoch + 1,
                'arch': args.arch,
