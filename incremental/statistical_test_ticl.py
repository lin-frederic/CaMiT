...
if __name__ == "__main__":
    ...
    for i, model in enumerate(static_models):
        for j, results_dir in enumerate(results_dirs):
            ...
            all_labels.append(f"{model} ({results_dir.split('_')[0]})")
    ...
    for i in range(n):
        for j in range(i + 1, n):
            heatmap1 = all_heatmaps[i]
            heatmap2 = all_heatmaps[j]
    ...
    # Plotting heatmaps
    plot_heatmap(wilcoxon_pvals, "Wilcoxon p-values", all_labels)
    plot_heatmap(permutation_pvals, "Permutation p-values", all_labels)
    plot_heatmap(ttest_pvals, "Paired t-Test p-values", all_labels)
    plot_heatmap(shapiro_pvals, "Shapiro-Wilk Normality Test p-values", all_labels)
