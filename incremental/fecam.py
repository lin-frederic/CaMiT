    ...
    with tqdm(total=len(train_years) * len(test_years)) as pbar:
        for train_year in train_years:
            ...
            for label in prototypes.keys():
                covariances[label] = torch.linalg.pinv(covariances[label])
            ...
            for test_year in test_years:
                ...
                acc = fecam(prototypes, covariances, test_embeddings, test_labels, batch_size=args.batch_size)
                results[train_year][test_year] = acc
                print(f"Train year: {train_year}, Test year: {test_year}, Accuracy: {acc:.4f}")
                pbar.update(1)
                
    # Save results
    os.makedirs("fecam_results", exist_ok=True)
    os.makedirs(f"fecam_results/{model_name}", exist_ok=True)
    with open(os.path.join(f"fecam_results/{model_name}", f"results_{args.job_id}.json"), "w") as f:
