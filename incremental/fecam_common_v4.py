...
def update_statistics(old_prototype, old_count, new_prototype, new_count):
    ...
    updated_count = n_total

    return updated_prototype, updated_count
...
if __name__=="__main__":
    ...
    with tqdm(total=len(train_years) * len(test_years)) as pbar:
        for train_year in train_years:
            ...
            # Update the statistics incrementally
            if average_covariance is None:
                average_covariance = covariance
            else:
                old_count = sum([counts[label] for label in counts.keys()])
                new_count = len(train_labels)
                average_covariance = (average_covariance * old_count + covariance * new_count) / (old_count + new_count)
            inv_covariance = normalize_cov(average_covariance)
            inv_covariance = torch.linalg.pinv(average_covariance)

            # Update the prototypes and counts incrementally
            ...
            for test_year in test_years:
                ...
                acc = fecam(averaged_prototypes, inv_covariance, test_embeddings, test_labels, batch_size=args.batch_size)
    ...
    # Save results
    os.makedirs("fecam_common_v4_results", exist_ok=True)
    os.makedirs(f"fecam_common_v4_results/{model_name}", exist_ok=True)
    with open(os.path.join(f"fecam_common_v4_results/{model_name}", f"results_{args.job_id}.json"), "w") as f:
