...
if __name__=="__main__":
    ...
    with tqdm(total=len(train_years) * len(test_years)) as pbar:
        for train_year in train_years:
            ...
            if averaged_covariance is None:
                averaged_covariance = covariance
            else:
                averaged_covariance = (averaged_covariance + covariance) / 2
            inv_covariance = normalize_cov(averaged_covariance)
    ...
    # Save results
    os.makedirs("fecam_common_v2_results", exist_ok=True)
    os.makedirs(f"fecam_common_v2_results/{model_name}", exist_ok=True)
    with open(os.path.join(f"fecam_common_v2_results/{model_name}", f"results_{args.job_id}.json"), "w") as f:
