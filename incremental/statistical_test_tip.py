...
def tip_results(mode, model_name="mocov3_s_lora"):
    ...
    years.insert(0, "2007")
    if model_name == "car_mocov3_s_259":
...
def plot_heatmap(matrix, title, xticklabels, mask_diag=True, cmap="viridis", annot=True):
    plt.figure(figsize=(10, 8))
    mask = np.eye(matrix.shape[0]) if mask_diag else None
    sns.heatmap(matrix, xticklabels=xticklabels, yticklabels=xticklabels,
                mask=mask, annot=annot, cmap=cmap, cbar_kws={"label": "p-value"})
    plt.title(title)
...
if __name__ == "__main__":
    ...
    print(avg_acc, future_acc, past_acc, current_acc)
    heatmap4, train_years, test_years, avg_acc, future_acc, past_acc, current_acc = tip_results(mode="", model_name="clip_b_lora")
    print(avg_acc, future_acc, past_acc, current_acc)
    heatmap5, train_years, test_years, avg_acc, future_acc, past_acc, current_acc = tip_results(mode="", model_name="mocov3_b_lora")

    n = 5
    wilcoxon_pvals = np.ones((n, n))
    permutation_pvals = np.ones((n, n))
    ttest_pvals = np.ones((n, n))
    shapiro_pvals = np.ones((n, n))
    heatmaps = [heatmap1, heatmap2, heatmap3, heatmap4, heatmap5]
    models = ["MoCo v3-S +R", "MoCo v3-S +L$_c$", "MoCo v3-S +R+L$_a$", "CLIP-B +L$_c$", "MoCo v3-B +L$_c$"]
    for i in range(n):
        for j in range(i+1, n):
            heatmap1 = heatmaps[i]
            heatmap2 = heatmaps[j]
            stats = statistical_test(heatmap1, heatmap2)
            wilcoxon_pvals[i, j] = wilcoxon_pvals[j, i] = stats["wilcoxon_p"]
            permutation_pvals[i, j] = permutation_pvals[j, i] = stats["permutation_p"]
            ttest_pvals[i, j] = ttest_pvals[j, i] = stats["ttest_p"] if stats["ttest_p"] is not None else np.nan
            shapiro_pvals[i, j] = shapiro_pvals[j, i] = stats["shapiro_p"]


    plot_heatmap(wilcoxon_pvals, "Wilcoxon Signed-Rank Test p-values", models)
    plot_heatmap(permutation_pvals, "Permutation Test p-values", models)
    plot_heatmap(ttest_pvals, "Paired t-Test p-values", models)
    plot_heatmap(shapiro_pvals, "Shapiro-Wilk Normality Test p-values (of differences)", models)
