...
from models import get_pretrain_ViT
...
def vit_base_patch16_224_adapter(pretrained=False, **kwargs):
    ...
    state_dict = replace_state_dict(state_dict, 768)
...
def resize_pos_embed(state_dict,model):
    ...
    model_num_patches = model_pos_embed.shape[1] - 1

    pretrained_class_token = pretrained_pos_embed[:, 0]
    pretrained_patch_embed = pretrained_pos_embed[:, 1:]
    
    model_class_token = model_pos_embed[:, 0]
    model_patch_embed = model_pos_embed[:, 1:]
    ...
    pretrained_patch_embed = pretrained_patch_embed.reshape(1, pretrained_grid_size, pretrained_grid_size, -1).permute(0, 3, 1, 2)
    model_patch_embed = model_patch_embed.reshape(1, model_grid_size, model_grid_size, -1).permute(0, 3, 1, 2)

    # Resize using bilinear interpolation
    resized_patch_embed = F.interpolate(pretrained_patch_embed, size=(model_grid_size, model_grid_size), mode='bicubic', align_corners=False)

    # Flatten back to 1D
    resized_patch_embed = resized_patch_embed.permute(0, 2, 3, 1).reshape(1, model_num_patches, -1)

    # Combine class token and resized patch embeddings
    resized_pos_embed = torch.cat([pretrained_class_token.unsqueeze(0), resized_patch_embed], dim=1)

    # Replace the positional embedding in the state dictionary
    state_dict['pos_embed'] = resized_pos_embed
    return state_dict

def dinov2_adapter(pretrain_model, **kwargs): # probably have to modify the architecture because there are layer scales in dinov2
    embed_dim = pretrain_model.embed_dim
    patch_size = pretrain_model.patch_embed.patch_size[0]
    depth = pretrain_model.blocks.__len__()
    num_heads = pretrain_model.num_heads
    model = VisionTransformer(patch_size=patch_size, embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    state_dict = pretrain_model.state_dict()
    state_dict = replace_state_dict(state_dict, embed_dim)
    ...
    msg = model.load_state_dict(state_dict, strict=False)
    print(msg)
    return set_grad(model, msg)

def mocov3_adapter(pretrain_model, **kwargs):
    embed_dim = pretrain_model.embed_dim
    model = VisionTransformer(patch_size=16, embed_dim=embed_dim, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    state_dict = pretrain_model.state_dict()
    state_dict = replace_state_dict(state_dict, embed_dim)
    if model.pos_embed.shape != state_dict['pos_embed'].shape:
...
def clip_adapter(pretrain_model, **kwargs):
    embed_dim = pretrain_model.embed_dim
    model = VisionTransformer(patch_size=16, embed_dim=embed_dim, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    print(model)
...
def replace_state_dict(state_dict, embed_dim):
...
if __name__=="__main__":
    model, preprocess = get_pretrain_ViT("clip_b")
    model = model.visual
    model.embed_dim = 768
    print(model)
    tuning_config = get_tuning_config(model)
    model = clip_adapter(model, tuning_config=tuning_config)
    exit()
    model = get_pretrain_ViT("car_mocov3_s_299")
    tuning_config = get_tuning_config(model)
    model = mocov3_adapter(model, tuning_config=tuning_config)
    #model = dinov2_adapter(model, tuning_config=tuning_config)
    #model = pretrain_model_adapter(model, tuning_config=tuning_config)