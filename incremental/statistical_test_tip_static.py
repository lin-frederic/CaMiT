...
if __name__ == "__main__":
    ...
    tip_heatmaps = [heatmap1, heatmap2, heatmap3, heatmap4, heatmap5]
    tip_models = ["MoCo v3-S +R", "MoCo v3-S +L$_c$", "MoCo v3-S +R+L$_a$", "CLIP-B +L$_c$", "MoCo v3-B +L$_c$"]
    
    static_models = [
        "dinov2_s", "car_mocov3_s_199", "mocov3_s_lora", 
        "dinov2_b", "clip_b", "clip_b_lora", 
        "car_mocov3_b_199", "mocov3_b_lora"
    ]
    static_heatmaps = []
    for i, model1 in enumerate(static_models):
        h1, *_ = results_to_heatmap("ncm_results", model1)
        static_heatmaps.append(h1)
    
    wilcoxon_pvals = np.ones((len(static_models), len(tip_models)))
    permutation_pvals = np.ones((len(static_models), len(tip_models)))
    ttest_pvals = np.ones((len(static_models), len(tip_models)))
    shapiro_pvals = np.ones((len(static_models), len(tip_models)))

    for i in range(len(static_models)):
        for j in range(len(tip_models)):
            heatmap1 = static_heatmaps[i]
    ...
    plot_heatmap(wilcoxon_pvals, "Wilcoxon Signed-Rank Test p-values", tip_models, static_models)
    plot_heatmap(permutation_pvals, "Permutation Test p-values", tip_models, static_models)
    plot_heatmap(ttest_pvals, "Paired t-Test p-values", tip_models, static_models)
    plot_heatmap(shapiro_pvals, "Shapiro-Wilk Normality Test p-values (of differences)", tip_models, static_models)