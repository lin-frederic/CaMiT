...
if __name__ == "__main__":
    ...
    year_models = [f"{model_name}_{year}_current" for year in years]
    years.insert(0, "2007")
    year_models.insert(0, model_name)
    
    #year_models = sorted(year_models, key=lambda x: int(x.split("_")[-1]) if x.split("_")[-1].isdigit() else 2007)
    ...
    plt.title("Evolution of Average Accuracies Over Years (Current)", fontsize=16)
    ...
    # Show the plot
    plt.tight_layout()
    plt.savefig("average_accuracies_over_years_current.png")
    plt.show()