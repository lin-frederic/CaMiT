...
def compute_prototypes_and_covariances(embeddings, labels):
    ...
    for label in tqdm(unique_labels, desc="Computing prototypes and second moments"):
        ...
        covariances[label] = torch.cov(normalized_embeddings.T) # (D, D)
        covariances[label] = shrink_cov(covariances[label])

    return prototypes, covariances


def update_statistics(old_prototype, old_covariance, old_count, new_prototype, new_covariance, new_count):
    """
    ...
    updated_covariance = (n_old * old_covariance + n_new * new_covariance) / n_total
    return updated_prototype, updated_covariance, n_total
...
if __name__=="__main__":
    ...
    with tqdm(total=len(train_years) * len(test_years)) as pbar:
        for train_year in train_years:
            ...
            prototypes, covariances = compute_prototypes_and_covariances(train_embeddings, train_labels)
            ...
            for label in prototypes.keys():
                if label not in averaged_prototypes:
                    ...
                    averaged_prototypes[label], averaged_covariances[label], counts[label] = update_statistics(averaged_prototypes[label],
                                                                                                               averaged_covariances[label],
                                                                                                                counts[label],
                                                                                                                prototypes[label],
                                                                                                                covariances[label],
    ...
    # Save results
    os.makedirs("fecamv4_results", exist_ok=True)
    os.makedirs(f"fecamv4_results/{model_name}", exist_ok=True)
    with open(os.path.join(f"fecamv4_results/{model_name}", f"results_{args.job_id}.json"), "w") as f:
