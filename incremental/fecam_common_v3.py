...
def compute_second_moment(embeddings):
    ...
    #second_moment = torch.mean(embeddings.unsqueeze(2) * embeddings.unsqueeze(1), dim=0) # (D, D)
...
if __name__=="__main__":
    ...
    with tqdm(total=len(train_years) * len(test_years)) as pbar:
        for train_year in train_years:
            ...
            # Compute global prototype and second moment
            ...
            # Compute covariance matrix
            inv_covariance = compute_covariance(averaged_global_prototype, averaged_global_second_moment)
            inv_covariance = torch.linalg.pinv(inv_covariance)
    ...
    # Save results
    os.makedirs("fecam_common_v3_results", exist_ok=True)
    os.makedirs(f"fecam_common_v3_results/{model_name}", exist_ok=True)
    with open(os.path.join(f"fecam_common_v3_results/{model_name}", f"results_{args.job_id}.json"), "w") as f:
