...
if __name__ == "__main__":
    ...
    for i, model1 in enumerate(static_models):
        for j in range(i + 1, len(static_models)):
            model2 = static_models[j]

            h1, *_ = results_to_heatmap("ncm_results", model1)
            h2, *_ = results_to_heatmap("ncm_results", model2)
            stats = statistical_test(h1, h2)
    ...
    def plot_heatmap(matrix, title, mask_diag=True, cmap="viridis", annot=True):
        plt.figure(figsize=(10, 8))
        mask = np.eye(matrix.shape[0]) if mask_diag else None
        sns.heatmap(matrix, xticklabels=static_models, yticklabels=static_models,
    ...
    plot_heatmap(wilcoxon_pvals, "Wilcoxon Signed-Rank Test p-values")
    plot_heatmap(permutation_pvals, "Permutation Test p-values")
    plot_heatmap(ttest_pvals, "Paired t-Test p-values")
    plot_heatmap(shapiro_pvals, "Shapiro-Wilk Normality Test p-values (of differences)")
