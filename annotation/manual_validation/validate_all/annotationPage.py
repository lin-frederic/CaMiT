...
def sort_images(images):
    return sorted(images, key=(lambda x: x[-3]), reverse=True)
...
def get_annotation_form():
    ...
    images_with_data = []
    for img_path, img_annotation in images.items():
        annotation_class = img_path.split("/")[-3]  
        car_probability = img_annotation.get("car_probability", 0)
        teacher_score = img_annotation.get("teacher_score", 0)
        gpt_student_score = img_annotation.get("gpt_student_score", 0)
        qwen_student_score = img_annotation.get("qwen_student_score", 0)
        box = img_annotation.get("box", [])
        img_url = path_to_url(img_path)
        images_with_data.append((img_url, img_path, box, annotation_class, teacher_score, gpt_student_score, car_probability, qwen_student_score))
...
def get_specific_annotation():
    ...
    images_with_data = []
    for img_path, img_annotation in full_dataset[cls][period].items():
        annotation_class = img_path.split("/")[-3]
        car_probability = img_annotation.get("car_probability", 0)
        teacher_score = img_annotation.get("teacher_score", 0)
        gpt_student_score = img_annotation.get("gpt_student_score", 0)
