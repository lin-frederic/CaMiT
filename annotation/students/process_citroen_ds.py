...
import argparse
import json
from dataset_cropvlm import CleanedGPTDataset, resize_transform, CitroenDS
...
parser.add_argument("--citroen_ds_annotations", type=str, help="Path to the citroen_ds annotations", default="validate_score/citroen_ds_annotations.json")
parser.add_argument("--negative_annotations", type=str, help="Path to the negative annotations", default="validate_score/negative_citroen_ds.json")
parser.add_argument("--positive_annotations", type=str, help="Path to the positive annotations", default="validate_score/positive_citroen_ds.json")
...
# Load the dataset
dataset = CleanedGPTDataset(args.citroen_ds_annotations, transform=transform)
negative_dataset = CitroenDS(args.negative_annotations, transform=transform)
positive_dataset = CitroenDS(args.positive_annotations, transform=transform)
...
for i in range(len(dataset)):
    ...
    if label == "citroen_ds":
        if label not in citroen_ds_indices:
            citroen_ds_indices[label] = []
        citroen_ds_indices[label].append(i)

negative_indices = {}
for i in range(len(negative_dataset)):
    label = negative_dataset.labels[i]
    if label == "citroen_ds":
        if label not in negative_indices:
            negative_indices[label] = []
        negative_indices[label].append(i)

positive_indices = {}
for i in range(len(positive_dataset)):
    label = positive_dataset.labels[i]
    if label == "citroen_ds":
        if label not in positive_indices:
            positive_indices[label] = []
        positive_indices[label].append(i)
...
negative_indices = negative_indices["citroen_ds"]
negative_dataset_subset = torch.utils.data.Subset(negative_dataset, negative_indices)
dataloader_negative = torch.utils.data.DataLoader(negative_dataset_subset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True)

positive_indices = positive_indices["citroen_ds"]
positive_dataset_subset = torch.utils.data.Subset(positive_dataset, positive_indices)
dataloader_positive = torch.utils.data.DataLoader(positive_dataset_subset, batch_size=args.batch_size, num_workers=args.workers, pin_memory=True)
positive_features = extract_feature(dataloader_positive)
negative_features = extract_feature(dataloader_negative)


indices = citroen_ds_indices["citroen_ds"]
...
with torch.no_grad():
    for instances, class_names, times, image_paths, image_names, box_indices, gpt_scores, qwen_scores in tqdm(dataloader):
        ...
        for i in range(len(instances)):
            ...
            assert class_name == "citroen_ds", f"Class name {class_name} does not match label citroen_ds"
            time = times[i]
...
print(f"Number of images in the dataset: {sum([len(image_data[time]) for time in image_data])}")

with open(args.citroen_ds_annotations, "r") as f:
    citroen_ds_annotations = json.load(f)
filtered_image_data = {}
for time in image_data:
    filtered_image_data[time] = {}
    for box_name, (feature, gpt_score, qwen_score) in image_data[time].items():
        feature_vec = np.expand_dims(feature, axis=0)
        pos_dist = cdist(feature_vec, positive_features, metric='cosine').min()
        neg_dist = cdist(feature_vec, negative_features, metric='cosine').min()
        if pos_dist < neg_dist:
            filtered_image_data[time][box_name] = (feature, gpt_score, qwen_score)

image_data = filtered_image_data            
print(f"Number of images in the dataset after filtering: {sum([len(image_data[time]) for time in image_data])}")
...
print([f"citroen_ds | {time}: {len(boxes)}" for time, boxes in time_to_sampled_boxes.items()])
# add to test images
...
with open(f"test_images/test_images_citroen_ds.json", "w") as f:
