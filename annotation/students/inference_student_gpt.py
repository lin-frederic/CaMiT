...
class TestInferenceStudentGPT(torch.utils.data.Dataset):
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('/home/users/flin/cars_time/finetune_gpt/best_checkpoint.pth', map_location=device)
    ...
    test_annotations = "outputs/test_gpt_annotations.json"

    test_dataset = TestInferenceStudentGPT(test_annotations, class_mapping, transform=transform)
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=8192, shuffle=False, num_workers=8) # for 80G GPU
    #test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=4096, shuffle=False, num_workers=4) # for 40G GPU
    ...
    if not os.path.exists("outputs/gpt_student_scores.json"):
        results = {}
        with torch.no_grad():
        ...
        with open("outputs/gpt_student_scores.json", "w") as f:
            json.dump(results, f)

        print("Inference done!")

    else:
        with open("outputs/gpt_student_scores.json", "r") as f:
            results = json.load(f)
