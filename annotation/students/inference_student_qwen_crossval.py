...
class TestInferenceStudentQwenCrossval(torch.utils.data.Dataset):
...
if __name__ == '__main__':
    ...
    test_annotations = "outputs/qwen_train_annotations.json"
    gpt_subset_indices = "outputs/gpt_subset"
    class_mapping = f"outputs/gpt_class_mapping.json"
    test_dataset = TestInferenceStudentQwenCrossval(test_annotations, class_mapping, transform=transform)
    ...
    os.makedirs("outputs/qwen_student_scores_crossval", exist_ok=True)
    for i in range(5):
        model = create_model(
        ...
        checkpoint = torch.load(f"/home/users/flin/cars_time/finetune_qwen_crossval{str(i)}/best_checkpoint.pth",
                                 map_location=device,
        ...
        #test_loader = torch.utils.data.DataLoader(test_dataset_i, batch_size=2048, shuffle=False, num_workers=4) # for 20G GPU
        ...
        if not os.path.exists(f"outputs/qwen_student_scores_crossval/qwen_student_scores_{str(i)}.json"):
            results = {}
            with torch.no_grad():
                for crops, class_ids, class_names, image_paths, box_indices in tqdm(test_loader, desc=f"Running Inference {str(i+1)}/5"):
                    crops = crops.to(device)
                    logits = model(crops) # (batch_size, num_classes)
            ...
            with open(f"outputs/qwen_student_scores_crossval/qwen_student_scores_{str(i)}.json", "w") as f:
                json.dump(results, f)
