...
class TestInferenceStudentQwen(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        class_name = box_annotations["class"]
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('/home/users/flin/cars_time/finetune_qwen/best_checkpoint.pth', map_location=device)
    model.load_state_dict(checkpoint['model'])
    model.eval()
    model.to(device)
    model = torch.compile(model)  # Optimize execution
    class_mapping = "outputs/gpt_class_mapping.json"
    #train_annotations = "outputs/train_gpt_annotations.json"
    test_annotations = "outputs/qwen_test_annotations.json"
    test_dataset = TestInferenceStudentQwen(test_annotations, class_mapping, transform=transform)
    #train_dataset = TestInferenceStudentGPT(train_annotations, class_mapping, transform=transform)
    #train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=2048, shuffle=False, num_workers=4)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=4096, shuffle=False, num_workers=8)
    #test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=8192, shuffle=False, num_workers=8)
    ...
    if not os.path.exists("outputs/qwen_student_scores.json"):
        results = {}
        with torch.no_grad():
            for crops, class_ids, class_names, image_paths, box_indices in tqdm(test_loader, desc="Running Inference"):
                ...
                for i, (image_path, box_index, class_id, class_name) in enumerate(zip(image_paths, box_indices, class_ids, class_names)):
                    box_path = image_path.replace(".jpg",f"_{str(box_index)}.jpg")
                    annotation_score = float(probs[i][class_id].item())
        ...
        with open("outputs/qwen_student_scores.json", "w") as f:
            json.dump(results, f)

        print("Inference done!")

    else:
        with open("outputs/qwen_student_scores.json", "r") as f:
            results = json.load(f)

    for i in range(1, 6):
        correct = 0
        for crop_path, result in results.items():
            gt_label, _ = result["annotation_score"]
